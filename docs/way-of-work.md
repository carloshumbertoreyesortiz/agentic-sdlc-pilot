...
**UAT-before-prod gate (US-067).** Telenor's Create_and_Review page is explicit: *"All acceptance criteria / test scenarios must be tested and documented in the GitHub issue. When all tests are documented and passed, the change can be promoted to production. Move the status in GitHub to Ready for Deployment."* The pilot enforces this at the CP2 → CP3 transition. A provenance record that **claims CP3** (a non-empty `cp3_approver.identity`) must also carry `uat_documented: true` **and** a non-empty `uat_evidence_url` (the link to the documented UAT results in the issue). The required `agent-provenance` gate enforces it: `scripts/validate-uat.ts` (pure logic in `src/uat.ts`) fails the gate on a CP3 claim without documented UAT — so no change reaches Ready for Deployment / Deployed without evidence. The gate is **inert for ordinary CP2 PRs** (those not claiming CP3), so it never blocks routine work. The `.github/ISSUE_TEMPLATE` Story/Task template carries a **UAT documentation** table (per-criterion scenario · result · date · tester) as the canonical evidence location, and `tools/uat-status.sh` surfaces the same pass/block decision locally for a readiness check before promotion.

**Approver routing and validation (US-066).** The specific approver at each checkpoint is resolved from [`docs/team-routing.yaml`](team-routing.yaml): CP1 and CP3 route to the initiative's **Change Lead**, CP2 to its **Technical Lead** (via `.github/CODEOWNERS`). Provenance records the CP1/CP3 approver identity (`cp1_approver` / `cp3_approver` fields) and the owning `initiative`. The required `agent-provenance` gate **enforces** this (US-066 Phase 2): `scripts/validate-approver.ts` runs on every `agent/*` PR and fails the gate if the CP1 approver is not authorized for the work's initiative, via `src/routing.ts` `validateApprover`. **Phase 0/1 exception:** the `agentic-pilot` initiative is solo-operated, so the sole operator (Carlos) passes regardless of the mapping — the same posture as R-01 (`enforce_admins=false`). Enforcement bites for any declared non-solo initiative (e.g. `initiative: sfb` once the SFB team joins), rejecting an approver who is not that initiative's Change Lead.
...
**Adoption risks.** The coordination and dependency risks this cadence exists to manage are tracked in [risks.md](risks.md): **R-TELENOR-CONCURRENT-MIGRATION** (four concurrent SFB migrations), **R-MATRIX-BASE-ACCESS** (no base Matrix access for the pilot — blocks everything downstream), **R-SERVICENOW-DEPENDENCY** (the dedicated integration user + scoped REST endpoint gating US-075), and **R-SFB-COORDINATION** (schema drift vs upstream #1121/#1595). Each carries an explicit exit trigger (US-077).
...
| Halvor Mortensen | ServiceNow-side governance contact | Sponsors / routes the Matrix **base-access** request and the **dedicated integration user + scoped REST endpoint** for US-075 (*not* the AIR role — see [technical-next-steps-matrix-sync.md](technical-next-steps-matrix-sync.md) Step 1b); referred Isak Charrad as incident-process owner | Email 2026-07-13; ServiceNow contact 2026-08-04 |
