...
**Approver routing and validation (US-066).** The specific approver at each checkpoint is resolved from [`docs/team-routing.yaml`](team-routing.yaml): CP1 and CP3 route to the initiative's **Change Lead**, CP2 to its **Technical Lead** (via `.github/CODEOWNERS`). Provenance records the CP1/CP3 approver identity (`cp1_approver` / `cp3_approver` fields), and `src/routing.ts` (`validateApprover`) checks the recorded approver against the initiative's Change Lead. **Phase 0/1 exception:** while an initiative is solo-operated, the sole operator (Carlos) is an authorized approver regardless of the mapping — the same posture as R-01 (`enforce_admins=false`); full role-match enforcement in the required `agent-provenance` gate activates when the SFB team joins (US-066 Phase 2, which depends on `team-routing.yaml` being on `main`).
