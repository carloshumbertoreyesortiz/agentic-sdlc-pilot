...
The pilot's dashboard (US-073) now shows a top-level cross-dashboard banner (`index.html`) with a one-line description of each dashboard and a link to the Salesforce **SFB Request Backlog** dashboard (id `01ZdV000000uD2bUAE`), which opens via Telenor SSO. The exact SF **My Domain** host in the link is pending confirmation from Ingrid (US-073 AC). The reverse link — Salesforce dashboard linking to the pilot's Pages URL — is out of scope for the pilot; it requires Salesforce admin work that Ingrid can arrange independently if she chooses.
