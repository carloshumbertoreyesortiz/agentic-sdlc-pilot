...
| **Matrix / ServiceNow** _(Flow C)_ | Matrix incidents (`matrix.telenor.no`) | Upstream — SFB team (#1595); **ServiceNow pushes to GitHub** via its own queue + outbound job (*not* the AIR role — that is for humans reporting incidents) | Pilot **receives** Matrix-sourced issues; sync owned by #1595 (see US-075) |
