...
11. **Publish the field-mapping contract** ([`matrix-github-field-mapping.md`](matrix-github-field-mapping.md)) — the exact JSON body ServiceNow should POST and the duplicate-prevention query. There is **no pilot-hosted receiver**: ServiceNow calls `api.github.com` directly, so what the pilot owes is a specification, and Halvor is currently building a sender without one.
