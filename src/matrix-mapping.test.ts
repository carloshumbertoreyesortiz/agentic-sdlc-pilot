...
describe('mapPriority', () => {
  it('maps the ServiceNow scale onto the pilot P-scale', () => {
    expect(mapPriority(1)).toBe('P1');
    expect(mapPriority(3)).toBe('P2');
    expect(mapPriority(5)).toBe('P4');
  });

  it('collapses Critical and High onto P1 — the lossy row Ingrid must confirm', () => {
    expect(mapPriority(1)).toBe(mapPriority(2));
  });
